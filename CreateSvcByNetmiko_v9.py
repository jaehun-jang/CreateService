...
def CheckFlexP(uType,MapUni): # ('eth' or 'roe') , '1/1'                    
 ...
 ### function to check a Type of Interface ###            
def CheckInterface(uType,MapUni): # ('eth' or 'roe') , '1/1'                    
    if uType == 'eth':
        uType = 'xe'
    elif uType == 'roe':
        uType = 'cpri'  
    output = connection.send_command("sh interface status")     
    lines = output.splitlines()
    # print(lines)
    for index, line in enumerate(lines):  
        columns = line.split() # To match the interface number correctly, ex 1/1 != 1/11
        if columns and columns[0] == MapUni:
            print(f"The line including {MapUni} is {index}.")
            original_string =lines[index].split()[1]
            # print(original_string)
            result_string = original_string[1:-1] # to remove "()" 
            # print(result_string)
            if result_string == uType:
                print(f"Int type {result_string} is matched with {uType}")
                return True
            else:
                print(f"Int type {result_string} is mismatched with {uType}")
                return False
...
def EplUNI(evc,uType,checkProduct):
    ...
    if checkProduct == 1:
        if CheckInterface(uType,MapUni) == False:
